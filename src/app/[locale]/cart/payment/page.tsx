  ...
  useEffect(() => {
    ...
    script.onload = async () => {
      ...
      try {
        ...
        console.error("Failed to create snap token", err);
      }
    };
    script.onerror = () => {
      ...
      <div className="flex-1 flex flex-col items-center justify-center py-24 px-6">
        ...
        <div className="flex gap-3">
          <button
            type="button"
            onClick={handlePay}
            className="bg-primary text-white h-12 px-8 rounded-md font-semibold text-sm hover:bg-primary-hover transition-colors cursor-pointer"
          >
            Pay Now
          </button>
          <button
            ...
            className="border border-gray-300 text-gray-700 h-12 px-6 rounded-md font-semibold text-sm hover:bg-gray-50 transition-colors cursor-pointer"
          >
            Back to Cart
          </button>
        </div>
